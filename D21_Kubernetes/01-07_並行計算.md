...
2. 新建脚本 `analyze_pi_perf.sh`：
    ...
    # 取得參數
    T_S="$1"                            # 單機耗時 (秒)
    NAMESPACE="${2:-pi-workload}"      # 預設命名空間
    JOB_NAME="${3:-pi-calculation-job}"# 預設 Job 名
...
如果你偏好 Python，也可以用類似方式寫一個小腳本，或直接將輸出導入 CSV，再用 Excel/Matplotlib 繪圖。這樣就完成了「多機效能比較」的全自動化！
