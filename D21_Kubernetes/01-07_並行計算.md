...
1. 使用小樣本快速驗證流程；運行後立刻會看到 `Start small test` 與計算結果，就能確定流程無誤。
...
5. 大樣本正式測試；在當前資料夾建立運算腳本檔 `pi_calc.py`，然後執行並使用 shell 內建 time 進行計時。
...
6. 最終會輸出單機耗時 `Tₛ`，同時存入變數 `T_S`；特別注意，這在當前視窗中有效。
...
觀看 `pi-calculation-job` 的 `COMPLETIONS` 欄位，直到顯示 `3/3`，才代表所有 Pod 都跑完。
...
你會看到三行新的 `Time: x.xx s`。



## 五、再執行分析腳本

最後，把剛才自動或手動測的 Tₛ（第一個參數）以及 kubeconfig、namespace、Job 名稱，當成參數傳給腳本：

```bash
~/analyze_pi_perf.sh auto ~/.kube/k3s.yaml pi-workload pi-calculation-job
```

* `auto`：腳本先在本機跑一次 `pi_calc.py` 得到最新的 Tₛ
* 之後它再去叢集抓剛才新跑的三個 Pod 耗時，計算平均、Speed-up、效率



如此，你就完成了一次「單機」（local）與「多機」（cluster）各自最新數據的全流程測試與比較。
