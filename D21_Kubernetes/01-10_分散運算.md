...
3. 將測試文件從本機傳送到各節點；以下指令會將片段依據分配規則傳送到 `red、blue、pi4`。
    ...
    ```bash
    scp test_part0{00..39}.mp4 red:~/Downloads/
    scp test_part0{40..79}.mp4 blue:~/Downloads/
    scp test_part0{80..99}.mp4 test_part10{0..9}.mp4 test_part11{0..5}.mp4 pi4:~/Downloads/
    ```

<br>

4. 先在 blue 及 pi4 進行連線測試，確認腳本不會出現錯誤。
...
2. 確保每個 Pod 只轉碼自己節點上的片段，轉碼後回傳至 red。
...
spec:
  ...
  template:
    spec:
      ...
      - name: ffmpeg
        ...
        args:
          - |
            ...
            ls -lh /home/sam6238/Downloads/

            echo "Processing video files..."
            for file in /home/sam6238/Downloads/test_part*.mp4; do
            ...
            scp -i /root/.ssh/id_ed25519 -o StrictHostKeyChecking=no /home/sam6238/Downloads/*_output.mp4 sam6238@192.168.1.158:/home/sam6238/Downloads/
        volumeMounts:
        - name: downloads
          mountPath: /home/sam6238/Downloads
        - name: ssh-keys
          mountPath: /root/.ssh
          readOnly: true
      restartPolicy: Never
      ...
      - name: ssh-keys
        hostPath:
          path: /home/sam6238/.ssh
          type: Directory
```

<br>

3. 刪除舊的 Job；若沒有 Job 會顯示 `Error from server (NotFound)`。
...
5. 監控運行狀態，會從 `ContainerCreating` 到 `Running`，然後 `Completed`，若有錯誤會顯示為 `Error`。
...
8. 如果 FFmpeg 仍在運行，可查看 Pod 佔用的資源。
...
9. 查詢指定狀態的容器，例如查詢是否還有活躍 `Running` 的 Container。
