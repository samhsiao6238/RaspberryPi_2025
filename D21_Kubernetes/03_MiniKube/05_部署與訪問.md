...
_關於以下操作會使用到的容器。若尚未建立，可先參考 `01_準備工作` 完成建立並上傳_
...
2. `template` 是定義了與 pod 相關的所有訊息，其內的 `containers` 指定了 pod 運行的所有容器；這裡使用前一個步驟所建立的鏡像 `<DockerHub-使用者帳號>/myapp-image-arm64`。
