...
2. 查詢並輸出 `eth0` 介面的連線名稱，同時存入變數 `CONNECTION_NAME` 中備用。
...
3. 查詢並輸出當前 `eth0` 的 IP 並存入變數 `CURRENT_IP` 中備用。
...
4. 使用以下指令將網路介面 `eth0` 設定為靜態 IP，通常可直接綁定在當前由 DHCP 分配的 IP 上；避免變數解析時出錯所以加上引號。
    ...
    ```bash
    sudo nmcli con modify "$CONNECTION_NAME" ipv4.addresses "$CURRENT_IP"
