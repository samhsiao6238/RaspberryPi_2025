...
2. 查詢並輸出 `wlan0` 介面的連線名稱，同時存入變數 `CONNECTION_NAME` 中備用。
...
1. 查看當前 default route 設定；有兩個 default 路由，系統會選擇路由優先級較低的，所以流量應該會走 eth0，但有可能產生雙重路由衝突。
...
2. 刪除 `wlan0` 的 `default route`；只有 `eth0` 擁有 `default route`，確保所有外部流量都走 `eth0`。
