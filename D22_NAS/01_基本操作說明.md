...
5. 在文件的下方加入以下內容；這些設定允許用戶在共享資料夾 `shared` 中進行讀寫；特別注意，`[Shared]` 中的 `path` 要將路徑展開，使用 `~/` 會出錯。
...
4. 接著依照以下操作順序，示範建立一個新的單一分割區。 

    ```bash
    o	建立新的空的 DOS 分割表（可選）
    d	刪除所有舊的分割區（可重複按幾次直到清除）
    n	建立新的分割區
    p	選擇主分割區（primary）
    1	分割區號為 1
    Enter	預設起始位置
...
3. 添加掛載點，其中 `sda1` 依據前面步驟查詢結果輸入；總之，這部分要根據自己的硬碟設備和文件系統類型調整。

    ```bash
    UUID=<貼上之前查詢到的結果> /mnt/nas ext4 defaults 0 2
    ```
...
5. 假如看到如下提示可以忽略，只是說 `/etc/fstab` 不需要 `systemctl daemon-reload`，因為 `mount -a` 會直接依據 `fstab` 最新內容執行。
