...
_也可以 [手動下載鏡像檔](https://www.raspberrypi.com/software/operating-systems/) 後進行安裝；不建議這樣操作，僅提供步驟作為參考_
...
1. 自定義 `主機名稱 hostname`，這將用於第一次的連線，請務必牢記；另外，主機名稱是包含尾綴 `.local` 的，連線時不要遺漏。

   ![](images/img_20.png)

<br>

2. 自訂使用者名稱及密碼，請務必牢記；特別注意，包含 `使用者密碼` 與 `WiFi 密碼` 在初次輸入時可以看到原始密碼，再次查看時會發現被展開為長密碼，這是因為系統使用了雜湊演算將密碼進行加密；僅看似不同、不影響實際設定值。
...
4. 預先設定好 WiFi，這樣在初次啟動時會自動連線路由器，如此便可使用無頭模式進行連線。

   ![](images/img_85.png)

<br>

5. WLAN 區域設置為 `TW`；特別說明，樹莓派的 WiFi 驅動會依據所選區域載入符合地區的頻段與功率設定，_若未選取_，可能導致設置系統啟動後未能正確將 WiFi 開啟而無法完成連線。

   ![](images/img_253.png)

<br>

6. 時區設置為 `Asia/Taipei`，鍵盤佈局設置為 `us`。
...
1. 勾選 `開啟 SSH`，並確認 `使用密碼登錄` 也同時被選取。

   ![](images/img_83.png)

<br>

2. 在可選頁籤中，可勾選 `提示音`；另外 `遙測` 部分可不用勾選，這應該是資訊蒐集的選項。
...
3. 在當前的燒錄器中存在 Bug，若是多次燒錄時，會出現尚在寫入階段就進入校驗，導致燒錄程序卡死，可將整個應用關閉重啟；特別說明，這可能僅僅是 UI 介面的錯誤，燒錄程序可能在背景中正確運行，總之基於穩健原則，還是重啟妥當。

   ![](images/img_254.png)

<br>

4. 正確進行燒錄會顯示進度，這個過程需要一點時間；燒錄完畢後，系統會依據設定 `進行驗證`，完成驗證後將卡片取出並插入樹莓派進行開機；特別注意，務必正確將卡片插入樹莓派後再連接電源開機。
