...
_到這裡為止已經開啟 VNC 並可透過 VNC 遠端連線到樹莓派的桌面，但以下先進行 VSCode 的 SSH 連線_

<br>

## 讀出文字

_簡單說明樹莓派系統如何讀出指定字串；若使用 MacOS 可直接運行 `say` 指令，至於 Win 系統則需用 PowerShell 搭配內建語音引擎朗讀文字_
...
5. 刪除指定主機的 Host Key，這裡以 `raspi-2025-blue.local` 為例。
