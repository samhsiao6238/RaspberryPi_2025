...
4. 讀取和發送公鑰：`客戶端` 會讀取 `本地私鑰`，並在本地資料夾 `~/.ssh` 中尋找對應的公鑰將其發送到 `伺服器` 進行比對；若對應的公鑰不存在於本地，則 SSH 會從私鑰進行計算並推導出對應的公鑰，相同的私鑰每次推導公鑰的結果必然一致。

<br>

5. 伺服器檢查公鑰：`伺服器` 檢查所接收到的公鑰是否存在 `~/.ssh/authorized_keys` 文件中。

<br>

6. 生成挑戰（Challenge）：如果存在且匹配成功，`伺服器` 會生成一個稱為 `挑戰（challenge）` 的隨機數據，並用公鑰加密數據後發送回 `客戶端`。
...
8. 伺服器驗證簽名：`伺服器` 驗證簽名是否正確，如果正確則身份驗證成功，`伺服器` 認為該連線請求是合法的，並允許連線。這個驗證過程確保私鑰不會離開本地主機，保證了私鑰的安全； `伺服器` 使用公鑰加密隨機數據，確保只有擁有正確私鑰的 `客戶端` 能夠解密。
