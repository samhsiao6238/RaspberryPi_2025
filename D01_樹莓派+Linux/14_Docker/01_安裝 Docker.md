...
2. 因為變更了使用者的群組，這等效於授權行為所以要刷新授權，可登出並重新登入樹莓派來實現；若不嫌麻煩重啟也是可以。
...
_補充說明前面操作食用魚網路請求的指令_

<br>

1. `curl` 是用於從 URL 取得或發送數據的終端機指令。

<br>

2. 參數 `-f` 或 `--fail` 表示當 HTTP 回應狀態碼為 `400` 或以上也就是失敗時，不輸出內容，並回傳錯誤碼。
...
4. 參數 `-S` 或 `--show-error`，與 `-s` 一起使用時表示在發生錯誤時顯示錯誤，也就是會隱藏正常的操作，僅顯示錯誤。
