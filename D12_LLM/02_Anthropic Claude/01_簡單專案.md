...
1. 本地檔案；可使用任意要查詢的知識如產品說明書等，格式可以是 `TXT/MD/CSV`，放在 `~/documents/knowledge/`。

    ```bash
    scp <本地文件路徑> sam6238@blue:~/Documents/knowledge
    ```
...
4. 編輯 `.env`；特別注意，這裡使用覆蓋指令 `>` 進行寫入，因為當前文件是空的，後續若要添加，則改用 `>>`。
    ...
    ANTHROPIC_API_KEY=<填入-ANTHROPIC-API-KEY>"
    > .env
    ```
...
4. 初次運行會下載模型；之後開始進行轉換，需要一點時間。

    ![](images/img_20.png)

<br>

5. 可將結果直接儲存成 `.txt` 檔，使用與來源同資料夾、同名。
...
6. 可改寫函數如下，將 PDF 轉文字後，自動存在同資料夾、同名的 .txt 檔案，調用方式不變。

    ```python
    import os

    def extract_pdf_with_ocr(pdf_path, lang=['ch_sim','en','ch_tra']):
        import pdfplumber
        import easyocr
        import numpy as np
...
_再次優化 `extract_pdf_with_ocr()`_

<br>

1. OCR 語言支援繁體中文、簡體中文、英文，每次 OCR 完成後，自動將結果存成同名 `.txt`，未來每次處理 PDF 前，先檢查是否已有同名 `.txt`，有就直接讀檔、跳過 OCR。
...
2. 調用時方式不變，再次運行就會發現不會進行轉換工作，因為同名的 `.txt` 文件已經存在。
