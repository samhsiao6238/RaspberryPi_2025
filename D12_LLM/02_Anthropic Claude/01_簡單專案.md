...
4. 初次運行會下載模型；之後開始進行轉換，特別注意，這是透過 AI 模型進行轉換，需要一點時間。

    ![](images/img_20.png)

<br>

5. 另外，訊息顯示 PDF 缺少 `CropBox` 設定，系統自動用 `MediaBox` 當作顯示區域；`MediaBox` 是 `PDF` 頁面的實體紙張範圍，也就是最大外框，`CropBox` 是 `PDF` 頁面實際顯示或裁切的範圍，也就是看到的區域。

    ![](images/img_24.png)

<br>

6. 可將結果直接儲存成 `.txt` 檔，使用與來源同資料夾、同名。
...
7. 可改寫函數如下，將 PDF 轉文字後，自動存在同資料夾、同名的 .txt 檔案，調用方式不變。
