...
2. 填入憑證
   ...
   * `LINE_CHANNEL_ACCESS_TOKEN`：在 LINE Developers Console → Channel → Messaging API → “Channel access token”
   * `TARGET_USER_ID`：可透過 Bot 的 webhook 事件中取得 `event.source.userId`
3. 執行測試

   * 啟動您的應用，呼叫 `GetUSDQuote` 方法時，應會在終端機看到

     ```
     [LINE Push] 訊息已發送
     ```
   * 同時指定的使用者會收到「這是一則固定內容的訊息」。

如此即可在 `_get_usd_quote` 被調用時，自動推送一則固定內容的 LINE 訊息。
