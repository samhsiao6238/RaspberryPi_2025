...
1. Python HTTP Server：透過以下指令可在 _運行指令的路徑中_ 快速啟動網站伺服器，若該目錄中沒有 `index.html`，則會建立一個靜態檔案伺服器，顯示執行該指令所在目錄的檔案清單，若目錄內有 `index.html` 就會自動顯示該超文本頁面。
...
2. 這是輕量 Web Framework，適合小型 API / 控制面板，透過以下指令安裝。
...
4. 貼上以下內容。
    ...
    @app.route('/')
    def home():
        return 'Hello from Flask on Raspberry Pi!'
...
6. 建立腳本 `call_api.py`，其中樹莓派 IP 為 `http://192.168.1.114/`。
...
9. 在腳本所在目錄中令開啟一個終端機，並運行 Python 腳本。
...
11. 查看自動產生的 Swagger UI API 文件，提供互動式 API 測試介面給開發者使用，可直接對 API 發送請求並查看回應。
...
12. 查看自動產生的 ReDoc API 文件，提供開發者閱讀 API 文件，設計風格與 Swagger 不同但資訊一致。
...
1. Express.js：這是最常見的 Node.js Web 框架，適合開發 REST API、Web App。

<br>

2. Next.js：基於 React 的 SSR（Server-Side Rendering）框架，搭配 Node.js 運行。 

<br>

3. Nuxt.js：基於 Vue 的 SSR 框架，搭配 Node.js 運行。

<br>

4. Koa.js：由 Express 團隊開發，更現代、輕量，支援 async/await。

<br>

5. NestJS：結合 TypeScript 的進階框架，適合大型應用，類似 Angular 架構。

<br>

6. Hapi.js：功能齊全、安全性高，適用於企業級應用。

<br>

7. Sails.js：類似於 Ruby on Rails，適合建立即時應用與資料導向的 API。
...
1. 後續步驟進行套件安裝時需使用 NPM，若尚未安裝則需先運行以下指令。
